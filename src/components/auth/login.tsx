...
import Link from "next/link";
import UniversalButton from "../ui/universalButton";
import { setClientCookie, removeClientCookie } from "@/services/cookieClient";
...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (rememberMe) {
      setClientCookie("SCRT", encrypt(password)); // env-driven attrs
      setClientCookie("USR", encrypt(pin));
    } else {
      removeClientCookie("SCRT");
      removeClientCookie("USR");
    }
